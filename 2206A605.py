...
class MLCourseGUI(QMainWindow):
    ...
    def load_dataset(self):
        ...
        try:
            ...
            if dataset_name == "Iris Dataset":
                ...
                self.show_error(f"Iris Loaded")
            elif dataset_name == "Breast Cancer Dataset":
                data = datasets.load_breast_cancer()
                self.show_error(f"Breast Cancer dataset Loaded")
            elif dataset_name == "Digits Dataset":
                data = datasets.load_digits()
                self.show_error(f"Digits Loaded")
            elif dataset_name == "Boston Housing Dataset":
                data = datasets.load_boston()
                self.show_error(f"Boston Loaded")
    ...
    def create_classical_ml_tab(self):
        ...
        Gnb_group = self.create_algorithm_group(
            "GaussianNB",
            {"var_smoothing": "double",
             " Prior Probabilities" : ["User-Defined", "Uniform"]}
        )
        ...
        try:
            if model_name == "Linear Regression":
               ...
               self.update_metrics(y_pred, loss)
                ...
                self.show_error(f"Model aint trained yet m8.")

        except Exception as e:
            self.show_error(f"Error training model. Again ")
